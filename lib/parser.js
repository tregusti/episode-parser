...
module.exports = function(filename) {
  ...
  if (result = parseSceneLikeSeason10(filename)) return result
...
function parseSceneLikeSeason10(filename) {
  var m, result = {}

  if (m = filename.match(/^(.+?)([\. ])(?:((?:20|19)\d\d)\2)?(10\d\d+)\2/i)) {
    var show = humanize(m[1])
        .replace(/\w\S*/g, function(txt){return txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase()})
    result.show =    show
    result.season =  +m[4].substr(0, 2)
    result.episode = +m[4].substr(-2)
